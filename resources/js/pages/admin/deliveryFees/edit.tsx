...
export default function DeliveryFeesEdit({ deliveryFee }: PageProps) {
    const { data, setData, put, processing, errors } = useForm<FormData>({
        ...
        free_shipping_threshold: deliveryFee.free_shipping_threshold || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('admin.delivery-fees.update', deliveryFee.id));
    };

    return (
        <AppLayout>
            <Head title="Modifier un frais de livraison" />
                ...
                <header className="sticky top-0 z-10 border-b border-slate-200 bg-white/80 backdrop-blur-xl">
                    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                        <div className="flex items-center justify-between">
                            <div>
                                <h1 className="text-2xl font-bold text-slate-900">Modifier un frais de livraison</h1>
                                <p className="text-sm text-slate-600">Modifiez les informations du frais de livraison</p>
                            </div>
